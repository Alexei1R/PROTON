...
//	vec3 lightDir = normalize(lpos - crntPos);
//
//
//	float diffuse = max(dot(norm,lightDir),0.0f);
//
//	float speculatLight = 0.5f;
//	vec3 viewDirection = normalize(camPos-crntPos);
//	vec3 reflectionDir = reflect(-lightDir,norm);
//	float speclamount= pow(max(dot(viewDirection,reflectionDir),0.0),20);
//	float specular = speclamount * speculatLight;
//
//
//
